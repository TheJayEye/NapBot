...
public class CommandHelp implements ICommand
{
	...
	public boolean execute(User user, TextChannel channel, String command, List<String> parameters, Message message) throws Exception
	{
		...
		output.add("-");
		output.add("-----------------------------------------------");
		output.add("**To set your sleep schedule:** Type `" + NapBot.CONFIGURATION.messagePrefix + "set` followed by a supported schedule name. For example, if you wanted to change your schedule to DC1, you would type `" + NapBot.CONFIGURATION.messagePrefix + "set DC1`. You may also specify a schedule variant by using a dash separator, e.g. `" + NapBot.CONFIGURATION.messagePrefix + "set DC1-extended` (supported variants are `shortened` `extended` `flipped` `modified` and `mutated`). If you are trying an experimental unlisted schedule, use `>set Experimental`.");
